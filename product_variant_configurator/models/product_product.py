...
class ProductProduct(models.Model):
    ...
    product_template_attribute_value_ids = fields.Many2many(
        'product.attribute.value', string='Template Attribute Values',
    ...
    def _compute_product_template_attribute_value_ids(self):
        for product in self:
            if self.product_tmpl_id.attribute_line_ids:
                ids = []
                for val in self.product_tmpl_id.attribute_line_ids:
                    ids += val.mapped('value_ids').ids
                product.product_template_attribute_value_ids = [(6, False, ids)]
            else:
    ...
    def onchange_product_tmpl_id(self):
        ...
        if value:
            return {'domain': {'attribute_value_ids': [('id', 'in', value.ids)]}}
