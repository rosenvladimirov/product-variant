    ...
    def default_get(self, default_fields):
        ...
        if (
            ...
            for attribute, values in groupby(
                ...
                ids = [x.product_attribute_value_id.id for x in values]
                _logger.info(f"LIST VALUES {attribute}::{list(values)} {ids}")
                attribute_ids.append(
                    Command.create(
                        {
                            "product_tmpl_id": product_tmpl_id.id,
                            "attribute_id": attribute.id,
                            "attribute_ids": [Command.set(ids)],
                            # "attribute_domain": [('id', 'in', ids)],
                        }
                    )
                )
            if not attribute_ids:
                UserError(
                    _(
                        "In template is not defined any attribute, maybe is single product"
                    )
                )
            result.update(
        ...
        if self.product_tmpl_id:
            ...
            if self.combination_indices:
                for attribute_line_ids in self.product_tmpl_id.attribute_line_ids:
                    attribute_id = self.product_template_attribute_value_ids.filtered(
                        lambda r: r.attribute_id.id
                        == attribute_line_ids.attribute_id.id
                    )
                    if (
                        attribute_id.product_attribute_value_id.id
                        not in attribute_line_ids.value_ids.ids
                    ):
                    ...
                    product_template_attribute_value_id = self.env[
                        "product.template.attribute.value"
                    ].search(
                        [
                            (
                                ...
                                Command.create(
                                    {
                                        ...
                                        "product_template_attribute_value_ids": [
                                            Command.set(attribute_value_ids)
                                        ],  # product.template.attribute.value
                                    }
                                )
                            ]
...
class ProductVariantCreateLines(models.TransientModel):
    ...
    attribute_ids = fields.Many2many('product.attribute.value', string='Attribute values')
    # attribute_domain = fields.Binary(string="Technical field for domain")
