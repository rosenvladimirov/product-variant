...
def get_rendered_default_code(product, mask, desc=False):
    ...
    for value in product.attribute_value_ids:
        ...
        if not desc and value.attribute_id.code:
            product_attrs[attr_name] += value.attribute_id.code
        elif desc and value.attribute_id.description_code:
            product_attrs[attr_name] += value.attribute_id.description_code
        if not desc and value.code:
            product_attrs[attr_name] += value.code
        elif desc and value.description_code:
            product_attrs[attr_name] += value.description_code

    all_attrs = extract_token(mask)
    missing_attrs = all_attrs - set(product_attrs.keys())
    if not desc:
        default_reference_missing_placeholder = product.env['ir.config_parameter'].sudo().get_param('default_reference_missing_placeholder')
    else:
        default_reference_missing_placeholder = product.env['ir.config_parameter'].sudo().get_param('default_description_reference_missing_placeholder')
    ...
    default_code = default_code.replace(default_reference_missing_placeholder, '')
    return default_code


def render_default_code(product, mask, desc=False):
    default_code = get_rendered_default_code(product, mask, desc=desc)
    if not desc:
        product.default_code = default_code.replace('.', '')
    else:
        product.description_short = default_code
...
class ProductTemplate(models.Model):
    ...
    reference_mask = fields.Char(
        ...
        help='Reference mask for building internal references of a '
             ...
             'attribute name')
    reference_description_mask = fields.Char(
        string='Variant description reference mask', copy=False)
    ...
    def create(self, vals):
        ...
        if (not vals.get('reference_description_mask') and product.attribute_line_ids or
                not self.user_has_groups(
                    'product_variant_default_code.group_product_default_code'
                )):
            vals['reference_description_mask'] = product._get_default_mask()
    ...
    def write(self, vals):
        ...
        if ('reference_description_mask' in vals and not vals['reference_description_mask'] or not
                self.user_has_groups(
                    'product_variant_default_code.group_product_default_code'
                )):
            if self.attribute_line_ids:
                vals['reference_description_mask'] = self._get_default_mask()
        result = super(ProductTemplate, self).write(vals)
        if vals.get('reference_mask') or vals.get('reference_description_mask'):
            ...
            for product in products:
                ...
                if product.reference_description_mask:
                    render_default_code(product, product.reference_description_mask, True)
        return result
...
class ProductAttribute(models.Model):
    ...
    description_code = fields.Char(
        string='Attribute Schort Description Code',
    )
...
class ProductAttributeValue(models.Model):
    ...
    def create(self, vals):
        if 'code' not in vals:
            ...
            vals['description_code'] = vals.get('name', '')[0:11]
